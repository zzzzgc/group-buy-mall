<template>
  <div class="container">
    <div style="width: 100%;" class="group" v-for="(groupBuy, groupBuyIndex) in detail.groupBuys" :key="groupBuyIndex">
      ...
      <div class="group-item" v-for="(order, orderIndex) in groupBuy.orderInfo" :key="orderIndex"
        ...
        <div>
          <div class="text-other">创建时间:{{order.createTime}}</div>
        </div>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      handleOrderClick: function (groupBuyIndex, orderIndex) { // 处理订单点击事件
        let order = this.detail.groupBuys[groupBuyIndex].orderInfo[orderIndex]
